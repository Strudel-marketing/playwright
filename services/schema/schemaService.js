...
async function extractSchema(url, options = {}) {
  ...
  try {
    ...
    const supportingTypes = Array.from(schemaTypes).filter(
      type =>
    ...
    const supportingPart =
      supportingTypes.length > 0
        ? `Supporting: ${supportingTypes.join(', ')}`
        : 'Supporting: None';
    ...
    const results = {
      ...
      main_entity: mainEntity
        ? { type: mainEntity.type, name: mainEntity.name, id: mainEntity.id }
        : null,
...
async function quickCheck(url) {
  ...
  try {
    ...
    const quickResults = await page.evaluate(() => {
      ...
      const hasWebPage =
        document.querySelector('[itemtype*="WebPage"]') !== null ||
    ...
    return { url, timestamp: new Date().toISOString(), results: quickResults };
